...
function init(options, type) {
  //type:1,表示bimviz引擎，type:2,表示forge引擎，type:3，表示bdviews引擎
  let _this = this;
  ...
  function initBDview() {
    ...
    setTimeout(() => {
      _this.engine.postMessage(
        {
          command: "Init",
          parameter: { menu: true, loadingFiles_display: false }
        },
        "*"
      );
      var para = "";
      para = _this.options.para;
      _this.engine.postMessage({ command: "SetMenuUrl", parameter: para }, "*");
    }, 2000);
  ...
  _this.highlightElementList = (list) => {
    if (_this.type === 2) {
      ...
      _this.engine.postMessage({command:"LookAtEntities",parameter:para},"*");
    }
  }
